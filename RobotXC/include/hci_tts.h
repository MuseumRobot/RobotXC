...
/* @{ */
	...
	*/
	 ...
	 * @brief 音节
	 */
	typedef struct _tag_TTS_SYLLABLE
	{
		/// 音节对应的文本
		const char *pszText;

		/// 音节对应的拼音
		const char *pszPronouciationText;

		/// 音节对应音频的字节长度
		unsigned int uiAudioBytes;
    ...
    typedef struct _tag_TTS_SYNTH_RESULT
    {
		...
		/// 动态配置，回调函数中可以设置 pszConfig 动态调整 TTS 配置。设置成 NULL 表示无配置调整
		...
		/// 音节数组
		TTS_SYLLABLE *pSyllable;

		/// 音节数组内音节的数量
		unsigned int uiSyllableCount;

		/// 音节数组对应的句子文本
		const char *pszSentence;

		/// 音节数组对应的句子文本的长度，字节为单位
		unsigned int uiSentenceLen;
    } TTS_SYNTH_RESULT;
 ...
 *		</tr>
 *		<tr>
 *			<td>reportProMode(仅tts.local.synth.v9支持)</td>
 *			<td>字符串，有效值{bySentence,byChar}</td>
 *			<td>bySentence</td>
 *			<td>按句/按字返回合成文本设置</td>
 *			<td>
 *				bySentence: 按句返回合成结果<br/>
 *				byChar: 按字返回合成结果<br/></td>
 *		</tr>
 *	</table>
 * @n@n
 * @par 本地背景音相关配置
